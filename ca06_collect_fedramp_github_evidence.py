...
def collect_repo(
    ...
    collaborators = paginate_json(
        session,
        f"/repos/{org}/{repo_name}/collaborators",
        params={"per_page": 100, "affiliation": "direct"},
        optional=True,
    )
    write_json(repo_dir / "collaborators.json", collaborators)
    manifest.append(f"repos/{repo_name}/collaborators.json")

    teams = paginate_json(session, f"/repos/{org}/{repo_name}/teams", params={"per_page": 100}, optional=True)
    write_json(repo_dir / "teams.json", teams)
    manifest.append(f"repos/{repo_name}/teams.json")
...
Notes:
- GitHub audit log access requires appropriate organization or enterprise privileges.
