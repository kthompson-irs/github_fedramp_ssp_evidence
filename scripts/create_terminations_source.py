#!/usr/bin/env python3
"""
Create data/terminations_source.csv from inline JSON or a source file.

This script is used to bootstrap the source roster file before the populate step.
It accepts either:
- --source <csv|json|jsonl>
- --json-env TERMINATIONS_JSON (a JSON array of rows)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create data/terminations_source.csv from source data.")
    parser.add_argument("--source", type=Path, default=None, help="Optional source file (.csv, .json, .jsonl).")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/terminations_source.csv"),
