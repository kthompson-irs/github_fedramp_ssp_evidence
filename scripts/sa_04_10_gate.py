...
This script checks:
- Code scanning alerts (CodeQL): fail on open High/Critical alerts
- Dependabot alerts: fail on open High/Critical alerts
...
def severity_is_blocking(severity: str, threshold: str) -> bool:
    ...
    current = severity_rank.get(severity.lower(), 0)
    limit = severity_rank.get(threshold.lower(), 3)
    return current >= limit and current > 0
