...
def build_findings(
    ...
    for repo in repos:
        ...
        dep_alerts = extract_alerts(dep)
        dep_counts = count_dependabot_severity(dep_alerts)

        if any(dep_counts.values()):
            top = "Low"
...
def build_sar(
    ...
    lines.append(
        "Assessment artifacts were collected using GitHub REST APIs and reviewed for configuration, vulnerability, and exposure indicators. Results were normalized into raw evidence files and summarized into this SAR."
    )
    lines.append("")
    ...
    lines.append("- Raw evidence directory: `raw/`")
    lines.append("- Traceability matrix: `CA02_CA03_Traceability.md`")
    lines.append("- POA&M workbook: `POAM_GitHub_Integration.xlsx`")
    lines.append("")
...
def collect() -> Tuple[Config, Dict[str, Any]]:
    ...
    if cfg.repos:
        ...
        repos_payload = paged_get(session, f"{API_BASE}/orgs/{cfg.org}/repos", params={"type": "all"})

    write_json(raw_dir / "repos.json", repos_payload)

    audit_log_payload = paged_get(
        session,
        f"{API_BASE}/orgs/{cfg.org}/audit-log",
        params={"phrase": cfg.audit_log_phrase} if cfg.audit_log_phrase else None,
        max_pages=10,
    )
    ...
    sar_md = build_sar(cfg, org_data, repos, audit_log_payload, findings)
