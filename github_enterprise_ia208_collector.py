name: FedRAMP IA-2(8) Enterprise Evidence Collection

on:
  workflow_dispatch:
    inputs:
      gh_enterprise:
        description: "Enterprise slug for audit-log collection"
        required: false
        type: string
      gh_days:
...
jobs:
  collect:
    ...
    env:
      ...
      GH_CHECKPOINT_FILE: .github/evidence_state/checkpoints/enterprise_audit_checkpoint.json
      GH_ARCHIVE_ROOT: evidence/archive
      GH_AUTO_PUSH_CHECKPOINT: "1"
      GH_ARCHIVE_S3_BUCKET: ${{ vars.GH_ARCHIVE_S3_BUCKET }}
      GH_ARCHIVE_S3_PREFIX: ${{ vars.GH_ARCHIVE_S3_PREFIX || 'github-enterprise' }}
    ...
    steps:
      ...
      - name: Validate required env vars
        run: |
          ...
          test -n "$GH_ENTERPRISE"

      - name: Run collector
        id: run_collector
        run: |
          set -euo pipefail
          output_dir="$(python github_enterprise_ia208_collector.py)"
      ...
      - name: Add summary
        ...
        run: |
          {
            echo "## FedRAMP IA-2(8) enterprise evidence collection"
            echo
            ...
            echo
            if [ -f "${{ steps.run_collector.outputs.output_dir }}/manifest.md" ]; then
              echo "### Manifest"
              echo '```'
              cat "${{ steps.run_collector.outputs.output_dir }}/manifest.md"
              echo '```'
            fi
      ...
      - name: Upload evidence artifact
        ...
        with:
          name: fedramp-ia208-enterprise-evidence
          path: ${{ steps.run_collector.outputs.output_dir }}
          if-no-files-found: error

      - name: Upload checkpoint artifact
        if: always() && hashFiles('.github/evidence_state/checkpoints/enterprise_audit_checkpoint.json') != ''
        uses: actions/upload-artifact@v4
        with:
          name: fedramp-ia208-enterprise-checkpoint
          path: .github/evidence_state/checkpoints/enterprise_audit_checkpoint.json
          if-no-files-found: error
        ...
        run: |
          ...
          if [ -f "$GH_CHECKPOINT_FILE" ]; then
            if ! git diff --quiet -- "$GH_CHECKPOINT_FILE"; then
              ...
              git commit -m "Update enterprise audit checkpoint" || true
              git push || true
            fi
          fi
