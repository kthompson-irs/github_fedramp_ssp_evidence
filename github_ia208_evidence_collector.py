...
This version reduces deep-pagination failures by:
...
- querying the audit log in small time windows instead of one huge crawl
- capping pages per window and total events
        ...
        self.session.headers.update(
            {
                ...
                "User-Agent": "fedramp-ia208-evidence-collector/4.0",
            }
        )
...
def collect_audit_log(
    ...
    for w_start, w_end in windows:
        ...
        created_filter = f"created:{w_start.isoformat()}..{w_end.isoformat()}"
        page = 1

        while page <= max_pages_per_window and len(collected) < max_events:
            params = {
                "include": "all",
            ...
            for event in data:
                ...
                key = (
                    ...
                    str(normalized.get("actor")),
                    str(normalized.get("org")),
                )
                if key in seen_keys:
                    continue
                seen_keys.add(key)
